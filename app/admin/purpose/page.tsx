...
export default function AdminPurposePage() {
  const [settings, setSettings] = useState<any>(emptyState);
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const res = await fetch('/api/admin/purpose');
        if (res.ok) {
          const data = await res.json();
          setSettings({ ...emptyState, ...data });
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert('Purpose page settings saved successfully.');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-headline italic font-bold text-white">Purpose Page Settings</h1>
          <p className="text-slate-400 text-sm mt-1">Edit hero and CTA content for the Purpose page.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
